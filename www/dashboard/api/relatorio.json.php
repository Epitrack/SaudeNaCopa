...
$sentimentos = $conn->ArrayResults();
	...
	//$total+=$dado["total"];
	 ...
	 //echo "<br>$campo12"; 
		...
		$arrayDevice = array();
		$arrayDevice[0] = (isset($dadosDevice[0]))? $dadosDevice[0] : array("id" => 0, "nome" => "android", "percent" => 0);
    	$arrayDevice[1] = (isset($dadosDevice[1]))? $dadosDevice[1] : array("id" => 0, "nome" => "desktop", "percent" => 0);
    	$arrayDevice[2] = (isset($dadosDevice[2]))? $dadosDevice[2] : array("id" => 0, "nome" => "ios", "percent" => 0);
    	$arrayDevice[3] = (isset($dadosDevice[3]))? $dadosDevice[3] : array("id" => 0, "nome" => "tablets", "percent" => 0);
		
		$retorno["campo_device_android"] = $arrayDevice[0]["percent"];
		$retorno["campo_device_ios"] = $arrayDevice[2]["percent"];
		$retorno["campo_device_pc"] = $arrayDevice[1]["percent"];
		$retorno["campo_device_tablet"] = $arrayDevice[3]["percent"];
