...
{  
	...
	function notLoggedSession($hash, $conn){
	...
	*/
	function validaLogin(){
		...
		$retorno = $this->login($login, $senha);
		if($retorno){
			echo json_encode(array("status" => true, "mensagem" => ""));
		}else{
			echo json_encode(array("status" => false, "mensagem" => $this->errormsg));	
		}
		//{"status":false,"mensagem":"Usuário já existe"}
	}
	...
	function login($username, $password)	
	{
		...
		if(!isset($_SESSION["auth_session"]))
		{
			...
			else 
			{
				...
				else 
				{
					...
					// Input is valid
					$query = $this->mysqli->prepare("SELECT isactive FROM users WHERE email = ? AND password = ?");
		...
		else 
		{
			...
			//$_SESSION["auth_session"] = md5(microtime());
			//$this->newsession($username);
	...
	function validaCadastroLogin($password, $email){

		include("auth.config.php");
		include("auth.lang.php");

		if(strlen($password) == 0) { $this->errormsg[] = $lang[$loc]['auth']['login_password_empty']; echo "4"; return false; }
		elseif(strlen($password) < 5) { $this->errormsg[] = $lang[$loc]['auth']['login_password_long']; echo "6"; return false; }
		
		else {

				$query = $this->mysqli->prepare("SELECT * FROM users WHERE email=? and password=?");
				...
				if($count >= 0) {
					...
					//echo "7";
					$this->LogActivity("UNKNOWN", "AUTH_REGISTER_FAIL", "Email ({$email}) not exists");
				
					$this->errormsg[] = $lang[$loc]['auth']['register_email_not_exist'];
	...
	function validaCadastro($apelido, $password, $verifypassword, $email){
		
		include("auth.config.php");
		include("auth.lang.php");

		if(strlen($apelido) == 0) { $this->errormsg[] = $lang[$loc]['auth']['login_username_empty']; echo "1"; return false; }
		elseif(strlen($apelido) > 30) { $this->errormsg[] = $lang[$loc]['auth']['login_username_long']; echo "2"; return false; }
		elseif(strlen($apelido) < 3) { $this->errormsg[] = $lang[$loc]['auth']['login_username_short']; echo "3"; return false; }
		elseif(strlen($password) == 0) { $this->errormsg[] = $lang[$loc]['auth']['login_password_empty']; echo "4"; return false; }
		elseif(strlen($verifypassword) == 0) { $this->errormsg[] = $lang[$loc]['auth']['login_password_empty']; echo "5"; return false; }
		elseif(strlen($password) < 5) { echo "5"; $this->errormsg[] = $lang[$loc]['auth']['login_password_long']; echo "6"; return false; }
		elseif($password != $verifypassword) { $this->errormsg[] = $lang[$loc]['auth']['login_password_long']; echo "7"; return false; }
	...
	*/
	...
	function register($username, $password, $verifypassword, $email, $ponto_monitoramento = "")
	{
		include("auth.config.php");
		include("auth.lang.php");
			...
			if(count($this->errormsg) == 0)
			{
				...
				else 
				{
					...
					else 
					{
						...
						$query = $this->mysqli->prepare("INSERT INTO users (username, password, email, activekey, isactive) VALUES (?, ?, ?, ?, ?)");
						$query->bind_param("sssss", $username, $password, $email, $activekey, $isactive);
						$query->execute();
						$query->close();
	...
	*/
	...
	function newsession($username)
	{
		...
		// Fetch User ID :		
		$ssql = "SELECT id, perfil_id FROM users WHERE email=?";
		$query = $this->mysqli->prepare($ssql);
		$query->bind_param("s", $username);
		$query->bind_result($uid, $perfil_id);
		$query->execute();
		$query->fetch();
		$query->close();
		...
		//setcookie("accessPerfil", $perfil_id);
		
		//echo $_COOKIE["auth_session"];
	}
	...
	*/
	...
	function notLogged($url) {
		if(!isset($_SESSION['auth_session']) || !$this->checksession($_SESSION['auth_session'])) {
			header("Location: $url");
		}
	}
	
	function logged($url) {
		if(isset($_COOKIE['auth_session']) && $this->checksession($_COOKIE['auth_session'])) {
			header("Location: $url");
		}
	}
	function logout($url="") {
		unset($_SESSION["auth_session"]);
		$sessaoAtiva = isset($_COOKIE['auth_session']);
		if($sessaoAtiva) {
			$this->deletesession($_COOKIE['auth_session']);
	...
	*/
	...
	function trataDataSite($data){
		$dataInicio = explode("-", $data);
		return $dataInicio[0] . "/" . $dataInicio[1] . "/" . $dataInicio[2];
	}
	...
	function setSession($dataInicio, $dataFinal, $validar,$inicio=true){
		session_start();
		
		$_SESSION["data_inicio_site"] = $dataInicio;
		$_SESSION["data_final_site"] = $dataFinal;
		$_SESSION["validar_data_site"] = $validar;
		
		$_SESSION["data_inicio"] = $this->trataDataBanco($dataInicio);
		$_SESSION["data_final"] = $this->trataDataBanco($dataFinal);
	...
	function verifyValidationPassword($pass, $confirPass) {
	...
	function getValorPontuacao(){
		...
		$query = $this->mysqli->prepare("SELECT * FROM jogos WHERE data_jogo=?");
		$query->bind_param("s", $dataAtual);
		$query->bind_result($db_key);
		$query->execute();
		$query->store_result();
		$count = $query->num_rows;
		...
		if($count >= 1){
			return array("pontos" => 3, "jogo_ok" => 1);
		}else{
			return array("pontos" => 1, "jogo_ok" => 0);
		}
	}
